...
$PAGE->requires->js("/theme/" . $PAGE->theme->name . "/javascript/user_lc_form.js");
$PAGE->requires->css("/theme/" . $PAGE->theme->name . "/css/user_lc_form.css");
if ($USER->id != $user->id) {
...
if ($userform->is_cancelled()) {
    ...
    /**
     * Небольшой костыль. С формы свойство "Пол" передается в виде массива
     * и, чтобы нормально отображался сохраненный пол пользователя на форме
     * необходимо массив перевести в числовое значение
     */
    if(isset($usernew->sex) && is_array($usernew->sex))
    {
        $usernew->sex = array_pop($usernew->sex);
    }
