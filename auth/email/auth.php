...
class auth_plugin_email extends auth_plugin_base {
    ...
    public function user_signup_with_confirmation($user, $notify=true, $confirmationurl = null) {
        ...
        /**
         * Добавление значения свойства username исходя из введенного Email-адреса
         *
         * @date 23.08.2018
         * @author Bad Wolf
         */
    ...
    function user_confirm($username, $confirmsecret) {
        global $DB, $SESSION;
        //TODO: Поправить эту хуету, проверить регистрацию и подтверждение на сервере
        $user = get_complete_user_data('username', $username);
        $DB->set_field("user", "confirmed", 1, array("id"=>$user->id));

        /**
         * Создание новой связи пользователя с ролью
         */
        $new_role_assignment_data = new stdClass();
        $new_role_assignment_data->roleid = 5;
        $new_role_assignment_data->contextid = 1;
        $new_role_assignment_data->userid = $user->id;
        $new_role_assignment_data->timemodified = time();
        $new_role_assignment_data->modifierid = $user->id;
        //$new_role_assignment_data->sortorder = 0;

        $DB->insert_record("role_assignments", $new_role_assignment_data, false);
